  ...
  create(){
    ...
    onOpen(){
      ...
      if(this.info.open == false){
        console.log("Is false .. Opening..");
        // set this.info.change to false
        this.info.change = false;
        // open
        this.elements.loc.classList.add("active");
        // set this.info.open to true
        this.info.open = true;

        // find input value
        let value = this.elements.input.value;
        // if input is valid (hsva)
        let valid = this.validValue(value);

        // Update results from Value
        this.setResultsFromValue(valid);
